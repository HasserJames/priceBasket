...
//This class contains a the collection of all offers available
...
object Offers {
  ...
  //Offer1. Apples have a 10% discount off their normal price this week
  val getApplesDiscount = (basket: Map[Product, Int]) => {
    val APPLES_DISCOUNT = 0.1 // TODO, get this from database or file
    ...
    val strDiscountDescription = if (appleDiscount > 0) {
      s"Apples 10% off: ${getAmountWithCurrency(appleDiscount)}"
    } else ""
    Discount(appleDiscount, strDiscountDescription)
  }

  //Offer2. Buy 2 tins of soup and get a loaf of bread for half price
  val getBreadLoafDiscount = (basket: Map[Product, Int]) => {
    ...
    val strDiscountDescription = if (breadLoafDiscount > 0) {
      s"Bread 50% off: ${getAmountWithCurrency(breadLoafDiscount)}"
    } else ""
